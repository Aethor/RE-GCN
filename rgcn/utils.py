...
def sort_and_rank_filter(batch_a, batch_r, score, target, all_ans):
    ...
    )  # indices : [B, 2] 第一列递增， 第二列表示对应的答案实体id在每一行的位置
    indices = indices[:, 1].view(-1)
    return indices
...
def stat_ranks(rank_list, method, log):  # added  eval_paper_authors log for logging
    ...
    for hit in hits:
        ...
        if log:  # added  eval_paper_authors log for logging
            # added  eval_paper_authors log for logging
            logging.debug(
                "Hits ({}) @ {}: {:.6f}".format(method, hit, avg_count.item())
            )
        ...
        for index in top_indices[_]:
            ...
            if r < num_rels:
                predict_triples.append(
                    [test_triples[_][0].item(), r.item(), index.item()]
                )  # MODIFIED  eval_paper_authors for memory reasons
                # predict_triples.append([test_triples[_][0], r, index]) # original version
            else:
                predict_triples.append(
                    [index.item(), r.item() - num_rels, test_triples[_][0].item()]
                )  # MODIFIED  eval_paper_authors for memory reasons
